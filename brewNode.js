...
const BrewNode = function(port) {
    ...
    // set up a listening socket server
    function init() {
        chain.init();
        ...
        brewServer.on('connection', connection => {
            ...
            // this will add the incoming connection to an array of socket connections
    ...
    const processedRecievedBlock = block => {
        ...
        if (block.previousHash == currentTopBlock.hash) {
            // Attempt to add the top block to our chain
            chain.addToChain(block);
            console.log('New block added');
            console.log(chain.getLatestBlock());
        }
        else {
            // It is ahead, therefore we are a few behind. Request the whole chain
    ...
    // remove disconnected peer from our collection of sockets
